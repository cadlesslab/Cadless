...
params = {
    ...
    "open_angle_deg": 120,  # angle between the two leaves about the pin axis
    "pin_radius": 2.45,  # 5 mm nominal pin, 0.05 mm radial running clearance
    "pin_overhang": 2,  # pin protrudes past both barrel ends
}
...
def build_pin(p):
    """Hinge pin down the barrel axis, protruding at both ends, with clearance."""
    return Pos(0, 0, -p["pin_overhang"]) * Cylinder(
        radius=p["pin_radius"],
        height=p["hinge_length"] + 2 * p["pin_overhang"],
        align=(Align.CENTER, Align.CENTER, Align.MIN))
...
# Swinging leaf: the two remaining bands, swung open about the pin axis.
leaf_swing = Rot(0, 0, params["open_angle_deg"]) * build_leaf(
    params, knuckle_bands=[1, 3], notch_bands=[0, 2, 4])

# Hinge pin through all five knuckles.
pin = build_pin(params)

# Positioned assembly: three disjoint bodies — the pin runs through every
# knuckle bore with 0.05 mm radial clearance and the leaves mesh with 0.2 mm
# axial gaps (no joints/constraints, positions only). Each body is added as
# its bare Solid so the Compound's geometry summary sees them all.
result = Compound(children=[leaf_fixed.solid(), leaf_swing.solid(), pin.solid()])
