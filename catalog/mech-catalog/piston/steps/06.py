...
boss_length = params["boss_y_outer"] - params["boss_y_inner"]
boss_center = (params["boss_y_inner"] + params["boss_y_outer"]) / 2
for side in (1, -1):
    boss = Cylinder(radius=params["boss_radius"], height=boss_length).rotate(Axis.X, 90)
    part += Pos(0, side * boss_center, params["pin_z"]) * boss

# Wrist-pin bore straight through both bosses and skirt walls, axis along Y.
pin_bore = Cylinder(radius=params["pin_bore_radius"], height=100).rotate(Axis.X, 90)
part -= Pos(0, 0, params["pin_z"]) * pin_bore

# Break the crown's sharp outer edge with a 1 mm chamfer.
crown_edges = part.edges().filter_by(GeomType.CIRCLE).group_by(Axis.Z)[-1]
part = chamfer(crown_edges, params["crown_chamfer"])
