...
params = {
    ...
    "window_height": 20,
    "window_y": 15,
    "mount_hole_radius": 2.1,  # M4 clearance, 4.2 mm diameter
    "mount_hole_dx": 46,
    "mount_hole_dy": 31,
}
...
window = Box(
    ...
    params["panel_thickness"] + 2,
    align=(Align.CENTER, Align.CENTER, Align.MIN),
)
part -= Pos(0, params["window_y"], -1) * window

# Four corner mounting holes.
for x in (-params["mount_hole_dx"], params["mount_hole_dx"]):
    for y in (-params["mount_hole_dy"], params["mount_hole_dy"]):
        hole = Cylinder(
            radius=params["mount_hole_radius"],
            height=params["panel_thickness"] + 2,
            align=(Align.CENTER, Align.CENTER, Align.MIN),
        )
        part -= Pos(x, y, -1) * hole
